# sdlcastg: vcpkg の ports/libvpx（2026-04-26 の fd2364dc）を写し、Android の ARM を
# NEON 付きで作るように直したもの。直したところには "sdlcastg:" と書いてある。
# 使い方は BUILD.md（--overlay-ports=sdlcastg/ports）。
...
if(VCPKG_TARGET_IS_WINDOWS AND NOT VCPKG_TARGET_IS_MINGW)
    ...
    elseif(VCPKG_TARGET_IS_ANDROID)
        # sdlcastg: ARM は libvpx の Android 用のターゲットで作り、NEON を使う
        # （元の generic-gnu だと SIMD なしの C だけになり、Pixel 7a で 720p の
        # エンコードに 1 コア半以上かかった。memo/cast.md の手順 4）。
        set(LIBVPX_TARGET "generic-gnu")
        # Settings
        if(VCPKG_TARGET_ARCHITECTURE STREQUAL x86)
        ...
        elseif(VCPKG_TARGET_ARCHITECTURE STREQUAL arm)
            ...
            # sdlcastg: --enable-thumb は付けない。NEON のアセンブリに GNU as の
            # -mimplicit-it=always が付き、clang が受け付けない。
        elseif(VCPKG_TARGET_ARCHITECTURE STREQUAL arm64)
            set(LIBVPX_TARGET "arm64-android-gcc")
        endif()
        # Set environment variables for configure
        # sdlcastg: configure は AS が "*clang" のときだけ -c を足す（".exe" 付きだと
        # 外れて、armv7 のアセンブリをリンクまでしようとする）。
        string(REGEX REPLACE "\\.exe$" "" _sdlcastg_as "${VCPKG_DETECTED_CMAKE_C_COMPILER}")
        set(ENV{AS} ${_sdlcastg_as})
        # sdlcastg: アセンブリにも clang のターゲットを渡す（configure は環境変数の
        # ASFLAGS に足していく）。無いと armv7 のアセンブリを x86 として扱う。
        set(ENV{ASFLAGS} "--target=${VCPKG_DETECTED_CMAKE_C_COMPILER_TARGET}")
        # sdlcastg: C と C++ も同じ。--extra-cflags は NEON の有無を調べたあとで
        # 足されるので、それだけだと arm64 の -march=armv8-a の試しが x86 向けに
        # なって失敗し、NEON が外れる。
        set(ENV{CFLAGS} "--target=${VCPKG_DETECTED_CMAKE_C_COMPILER_TARGET}")
        set(ENV{CXXFLAGS} "--target=${VCPKG_DETECTED_CMAKE_CXX_COMPILER_TARGET}")
